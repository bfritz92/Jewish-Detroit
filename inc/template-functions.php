...
function deregister_scripts(){
	global $post;
		$use_gf = get_post_meta( $post, 'use_gf', true );
	//Change this conditional to target whatever page or form you need.
	/*if ( has_block( 'acf/donation-block', $post ) ||  is_page('Events') ) { */
/*	if ( !is_page_template( 'donate.php') || !is_page('Events') || !$use_gf ) {
...
// JFMD - Adds close button link to beginning of SideNav Menu.
function addCloseMenuLink($menuItems, $args)
{
    if('header-sidenav' == $args->theme_location)
    { 
        $homeMenuItem = '<li>' .
                        $args->before .
...
add_filter( 'wp_nav_menu_items', 'addCloseMenuLink', 10, 2 );	


// JFMD - Adds search to end of Main Menu.
...
function addSearchMenuLink($menuItems, $args)
{
    ...
    { 
        $homeMenuItem = '<li class="top-level-nav">' .
                        $args->before .
                        '<a tabindex="0" onclick="openSearch()" aria-label="Search">' .
                            $args->link_before .
...
add_filter( 'wp_nav_menu_items', 'addSearchMenuLink', 10, 2 );	

// JFMD - Adds open to end of Mobile Donate Menu.

function addOpenMenuLink($menuItems, $args)
{
    if('header-donate' == $args->theme_location)
    { 
        $homeMenuItem = '<li class="menu-toggle">' .
                        $args->before .
                        '<a href="javascript:void(0)" id="mobileMenu" aria-label="Menu" onclick="openNav()" >' .
                            $args->link_before .
...
add_filter( 'wp_nav_menu_items', 'addOpenMenuLink', 10, 2 );	

// JFMD - Adds classes to UL items Main Menu.
function change_ul_item_classes_in_nav( $classes, $args, $depth ) {
    if ( 0 == $depth ) {
        $classes[] = 'second-level-nav';
    }
    if ( 1 == $depth ) {
        $classes[] = 'third-level-nav';
    }
    // ...
    return $classes;
}
...
function add_specific_menu_location_atts( $atts, $item, $args ) {
    ...
    if( $args->theme_location == 'header-mainnav' ) {
      // add the desired attributes:
      $atts['class'] = 'second-level-nav--link';
    }
    return $atts;
}
...
add_action( 'wp_enqueue_scripts', 'rest_api_enqueue_styles' );
 function rest_api_enqueue_styles() {    
    ...
    wp_enqueue_script( 'rest-theme-js', get_stylesheet_directory_uri() . '/js/rest-api.js', array( 'jquery' ) );
...
/* Import Events from Event Calendar (https://jewishdetroitcalendar.org) | End */
